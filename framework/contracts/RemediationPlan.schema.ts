...
export const RemediationPhaseSchema = z.object({
  ...
  validationSteps: z.array(ValidationStepSchema).min(3, 'Each phase must include build, test, and lint validation'),
...
export const RemediationPlanSchema = z.object({
  ...
  phases: z
    ...
    .refine(phases => {
      ...
      return firstPhase?.validationSteps.some(
        step =>
  ...
  complianceTargets: z.array(ComplianceTargetSchema).min(1, 'Plan must define measurable compliance targets'),
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      const criticalIssues = error.issues.filter(err =>
        err.path.some(
          segment =>
            typeof segment === 'string' && ['rollbackPlan', 'validationSteps', 'dryRunSimulation'].includes(segment)
        )
      );
