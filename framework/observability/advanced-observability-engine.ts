...
class AdvancedObservabilityEngine {
  ...
  async analyzeSystemHealth(): Promise<{
    ...
    const overallHealth = Object.values(componentHealth).reduce((sum, score) => sum + score, 0) / Object.keys(componentHealth).length;
  ...
  private calculateAILearningHealth(events: TelemetryEvent[]): number {
    const learningEvents = events.filter(e => 
      e.category === 'pattern-recognition' || 
      e.category === 'predictive-enforcement' ||
      e.category === 'learning'
    );
  ...
  private async analyzeTrends(metrics: MetricSnapshot[]): Promise<TrendAnalysis[]> {
    ...
    // Performance trend
    const perfChange = latest.metrics.performance.averageResponseTime - previous.metrics.performance.averageResponseTime;
    trends.push({
  ...
  private calculateGovernanceScore(metrics: GovernanceMetrics): number {
    return (metrics.democraticParticipation + metrics.amendmentSuccessRate + 
            metrics.consensusQuality + metrics.constitutionalStability) * 25;
  }
...
async function main() {
  ...
  switch (command) {
    ...
    case 'metrics':
      ...
      console.log(`Governance: ${(snapshot.metrics.governance.democraticParticipation * 100).toFixed(1)}% participation`);
      break;
