...
describe('Apprenticeship Scaffolds Replay', () => {
  it('should generate identical apprenticeship CLI output on replay', () => {
    const output1 = execSync('node cli/apprenticeship-cli.cjs --mode guided --blueprint blueprints/feat-public-viewing/blueprint.yaml').toString();
    const output2 = execSync('node cli/apprenticeship-cli.cjs --mode guided --blueprint blueprints/feat-public-viewing/blueprint.yaml').toString();
