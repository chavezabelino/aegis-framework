...
- CI SHALL recompute `sha256(body)` with LF line endings and with the `@hash` line excluded, and SHALL verify an HMAC signature using `$AEGIS_HMAC_KEY` written to `.aegis/attestations/<commit>/<relpath>.sig`. Any mismatch MUST fail CI.
...
- PREC files MUST:
  ...
  - List commands CI MUST execute, and expected outputs CI MUST verify: existence, size > 0, and modification time > job start, plus sha256 hash.
