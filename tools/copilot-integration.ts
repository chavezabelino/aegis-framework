 ...
 */
export async function copilotChatHook(userMessage: string, chatContext?: any): Promise<void> {
  // Extract workspace from chat context if available
  const workspaceRoot = chatContext?.workspaceFolder?.uri?.fsPath;

  await captureConversationContext(userMessage, undefined, workspaceRoot);
}

/**
 * Hook for GitHub Copilot completion integration
 */
export async function copilotCompletionHook(documentContext: string, triggerText: string): Promise<void> {
