   ...
   for (const a of m.artifacts || []) {
     ...
     if (!res.ok) {
       if (isSig && relaxSigChecks) {
         findings.push({ level: 'warning', message: `Missing signature (relaxed): ${a.path}`, manifest: manifestPath, file: a.path });
       } else {
         findings.push({ level: 'error', message: `Required output file not found: ${a.path}`, manifest: manifestPath, file: a.path });
       }
     }
   }
   return outputs;
 ...
 function runCommands(manifestPath: string, cmds: EvidenceCommand[]) {
   ...
   for (const c of cmds) {
     ...
     if (localFindings.length) {
       findings.push({ level: 'error', message: `Command failed expectations: ${c.run} :: ${localFindings.join('; ')}`, manifest: manifestPath });
     }
