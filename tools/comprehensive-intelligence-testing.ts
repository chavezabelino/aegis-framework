...
export class ComprehensiveIntelligenceTesting {
  ...
  private async runSecurityTest(test: IntelligenceTest): Promise<any> {
    ...
    try {
      ...
      const score = securityFindings.filter(f => f.severity === 'critical' || f.severity === 'high').length === 0 ? 85 : 60;
  ...
  private generateTestRecommendations(results: TestResult[]): string[] {
    ...
    const performanceIssues = results.filter(r => 
      r.performanceMetrics && r.performanceMetrics.responseTime > 5000
    );
    ...
    const securityIssues = results.filter(r => 
      r.securityFindings && r.securityFindings.some(f => f.severity === 'critical' || f.severity === 'high')
    );
  ...
  private identifyCriticalIssues(results: TestResult[]): string[] {
    ...
    const criticalFailures = results.filter(r => 
      ...
      this.tests.get(r.testId)?.criticalityLevel === 'critical'
    );
    ...
    const criticalSecurityFindings = results.filter(r => 
      r.securityFindings && r.securityFindings.some(f => f.severity === 'critical')
    );
