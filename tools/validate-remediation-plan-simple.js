...
function validateRemediationPlan(planPath) {
  ...
  try {
    ...
    const requiredFields = ['id', 'name', 'description', 'version', 'aegisFrameworkVersion', 'intent', 'context', 'mode'];
    ...
    if (plan.phases && plan.phases.length > 0) {
      ...
      if (firstPhase && firstPhase.validationSteps) {
        const hasRequiredValidation = ['build', 'test', 'lint'].some(type =>
          firstPhase.validationSteps.some(step =>
            step.name.toLowerCase().includes(type) || step.command.toLowerCase().includes(type)
          )
        );
