...
try {
  ...
  const hasIntentEnforcement = scaffoldedTools.some(tool => 
    tool.name === 'intent-enforcement-engine'
  );
  const hasConstitutionalAgent = scaffoldedTools.some(tool => 
    tool.name === 'constitutional-ai-agent'
  );
