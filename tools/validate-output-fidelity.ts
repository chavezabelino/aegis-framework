...
interface FileDifference {
  ...
  expected: string;
  actual: string;
  type: 'content' | 'encoding' | 'whitespace';
}
...
class OutputFidelityValidator {
  ...
  private violations: FidelityViolation[] = [];

  constructor() {
    this.frameworkRoot = path.resolve(__dirname, '..');
  }

  async validateAll(): Promise<FidelityValidationResult> {
    ...
    const result: FidelityValidationResult = {
      overall,
      matches,
  ...
  private async validateOutputs(): Promise<ReferenceMatch[]> {
  ...
  private async compareFiles(outputPath: string, referencePath: string): Promise<ReferenceMatch> {
    ...
    const encodingIssues: EncodingIssue[] = [];
  ...
  private classifyDifference(actual: string, expected: string): 'content' | 'encoding' | 'whitespace' {
  ...
  private detectEncodingIssue(actual: string, expected: string, line: number): EncodingIssue | null {
  ...
  private calculateOverallScore(matches: ReferenceMatch[]): number {
  ...
  private printResults(result: FidelityValidationResult): void {
...
// CLI execution
const isMainModule = process.argv[1] && process.argv[1].endsWith('validate-output-fidelity.ts');
if (isMainModule) {
  const validator = new OutputFidelityValidator();
  validator.validateAll().catch(console.error);
}

export { OutputFidelityValidator, type FidelityValidationResult };
