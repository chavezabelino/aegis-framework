...
program
  .name('aegis-memory')
  .description('Constitutional memory governance for Aegis Framework')
  .version('1.0.0');
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      spinner.fail(`Failed to initialize memory subsystem: ${error.message}`);
      process.exit(1);
    }
  });
    ...
    new Command('append')
      ...
      .action(async (content, options) => {
        ...
        try {
          ...
          spinner.fail(`Failed to append message: ${error.message}`);
        }
      })
  )
  .addCommand(
    new Command('context')
      ...
      .action(async (options) => {
        ...
        try {
          ...
          const messages = fs.readFileSync(messageLog, 'utf8')
            .trim()
            .split('\n')
          ...
          spinner.fail(`Failed to generate context: ${error.message}`);
        }
      })
  )
  .addCommand(
    new Command('summarize')
      ...
      .action(async (options) => {
        ...
        try {
          ...
          spinner.fail(`Failed to summarize lite memory: ${error.message}`);
        }
      })
  );
    ...
    new Command('commit')
      ...
      .action(async (snapshotId, options) => {
        ...
        try {
          ...
          spinner.fail(`Failed to commit snapshot: ${error.message}`);
        }
      })
  )
  .addCommand(
    new Command('list')
      ...
      .action(async (options) => {
        ...
        try {
          ...
          spinner.fail(`Failed to list memory items: ${error.message}`);
        }
      })
  );
...
program
  ...
  .action(async (options) => {
    ...
    console.log(chalk.gray(`  Storage: ${memoryState.heavyMemory.storageQuota.used}/${memoryState.heavyMemory.storageQuota.limit} bytes`));
