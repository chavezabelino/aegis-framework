 ...
 * @version: 2.5.0
 * @mode: lean
 * @intent: CLI tool for planning optimization commands
 * @context: Easy access to planning optimization features
 * @model: claude-3-5-sonnet
 * @hash: 7f6a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a3
 */
...
import { AutoPlanDetector } from '../tools/auto-plan-detector.ts';
import { PlannerCritic } from '../tools/planner-critic.ts';
...
class PlanningCLI {
  ...
  private setupCommands(): void {
    this.program.name('aegis-planning').description('Aegis Planning Optimization CLI').version('1.0.0');
      ...
      .action(async (prompt: string, options: { output: string; verbose: boolean }) => {
        try {
          ...
          const plan = {
            ...
            risks: ['Middleware might affect other routes', 'Authentication state management complexity'],
            rollback: ['revert src/middleware/auth.ts', 'revert src/pages/login.tsx'],
            doneWhen: ['Contracts pass locally', 'Redirects accept /login or /(auth)/login', 'plan-gate passes'],
          };

          // Validate plan
          const validation = PlanSchema.safeParse(plan);
          if (!validation.success) {
            console.error('❌ Plan validation failed:', validation.error.issues);
            process.exit(1);
          }
      ...
      .action(async (planClass: string, filePath: string, filesTouched: string) => {
        try {
          ...
          if (!planValidation.success) {
            console.error('❌ Plan schema validation failed:', planValidation.error.issues);
            process.exit(1);
          }
          ...
          if (validationResult.estimatedTokens > constraint.maxTokens) {
            validationResult.errors.push(
              `Plan exceeds token limit: ${validationResult.estimatedTokens}/${constraint.maxTokens}`
            );
          ...
          if (validationResult.filesTouched > constraint.maxFiles) {
            validationResult.errors.push(
              `Plan exceeds file limit: ${validationResult.filesTouched}/${constraint.maxFiles}`
            );
            validationResult.valid = false;
          }
    ...
    this.program
      ...
      .option('--class1 <class>', 'First plan class', 'MVP-Fix')
      .option('--class2 <class>', 'Second plan class', 'MVP-Fix')
      .option('--files1 <count>', 'First plan file count', '2')
      .option('--files2 <count>', 'Second plan file count', '2')
      .action(
        async (
          file1: string,
          file2: string,
          options: { class1: string; class2: string; files1: string; files2: string }
        ) => {
          try {
            // Read plan files
