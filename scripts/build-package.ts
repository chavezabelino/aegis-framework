...
class PackageBuilder {
  ...
  private generatePackageMetadata(): void {
    ...
    fs.writeFileSync(path.join(this.distDir, 'packages.json'), JSON.stringify(metadata, null, 2));
...
`;
  ...
  private copyNodeModules(cliDir: string): void {
    ...
    fs.writeFileSync(path.join(nodeModulesDir, 'package.json'), JSON.stringify(nodeModulesPackage, null, 2));
  }
  ...
  private createCLIPackageJson(cliDir: string): void {
    const cliPackage = {
      ...
      keywords: ['ai', 'framework', 'blueprint', 'governance', 'constitutional', 'hydration'],
      license: 'MIT',
    ...
    fs.writeFileSync(path.join(cliDir, 'package.json'), JSON.stringify(cliPackage, null, 2));
  }
  ...
  private generateChecksums(): Record<string, string> {
    ...
    try {
      ...
      if (fs.existsSync(cliDir)) {
        const cliChecksum = execSync(`find "${cliDir}" -type f -exec sha256sum {} \\; | sha256sum`, {
          encoding: 'utf8',
        });
      ...
      if (fs.existsSync(libDir)) {
        const libChecksum = execSync(`find "${libDir}" -type f -exec sha256sum {} \\; | sha256sum`, {
          encoding: 'utf8',
        });
